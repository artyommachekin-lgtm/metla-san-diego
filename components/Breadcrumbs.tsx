                ...
                <ol className="flex items-center flex-wrap text-sm" itemScope itemType="https://schema.org/BreadcrumbList">
                    {breadcrumbItems.map((item, index) => (
                        <li key={index} className="flex items-center" itemProp="itemListElement" itemScope itemType="https://schema.org/ListItem">
                            {index > 0 && <ChevronRight className="w-4 h-4 mx-2 text-slate-400" aria-hidden="true" />}
                            {item.href ? (
                                <Link href={item.href} className="text-teal-600 hover:text-teal-700 flex items-center" itemProp="item">
                                    {index === 0 && <Home className="w-4 h-4 mr-1" />}
                                    <span itemProp="name">{item.label}</span>
                                </Link>
                            ) : (
                                <span className="text-slate-600 font-medium" itemProp="name">{item.label}</span>
                            )}
