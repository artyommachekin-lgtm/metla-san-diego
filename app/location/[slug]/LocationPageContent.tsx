...
const LocationPageContent: React.FC<LocationPageContentProps> = ({ slug }) => {
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="p-5 bg-slate-50 rounded-xl">
                    ...
                    <p className="text-sm text-slate-600">
                      We show up on time, every time. We use systems, not guesswork &mdash; so your {location.name} home is cleaned on schedule.
                    </p>
                  </div>
                  <div className="p-5 bg-slate-50 rounded-xl">
                    <div className="flex items-center gap-3 mb-2">
              ...
              {(location.landmarks || location.neighborhoods || location.housingTypes) && (
                <div className="mb-12">
                  ...
                  {location.housingTypes && location.housingTypes.length > 0 && (
                    <div className="mb-6">
                      <h3 className="text-lg font-bold text-slate-900 mb-3">Property Types We Clean</h3>
                      <div className="flex flex-wrap gap-2">
                        {location.housingTypes.map((type, i) => (
                          <span key={i} className="px-3 py-1 bg-teal-50 text-teal-700 rounded-full text-sm">{type}</span>
                        ))}
                      </div>
                    </div>
                  )}
                  ...
                  {location.landmarks && location.landmarks.length > 0 && (
                    <div className="mb-6">
                      <h3 className="text-lg font-bold text-slate-900 mb-3">Key Landmarks Near Our Service Routes</h3>
                  ...
                  {location.neighborhoods && location.neighborhoods.length > 0 && (
                    <div className="mb-6">
                      <h3 className="text-lg font-bold text-slate-900 mb-3">{location.name} Areas Served</h3>
                      <div className="flex flex-wrap gap-2">
                        {location.neighborhoods.map((area, i) => (
                          <span key={i} className="px-3 py-1 bg-slate-100 text-slate-700 rounded text-sm">{area}</span>
                        ))}
                      </div>
                    </div>
                  )}
              ...
              <div className="mb-12">
                ...
                <div className="space-y-4">
                  <div className="p-5 border border-teal-200 rounded-xl bg-teal-50/30">
                    ...
                    <Link
                      href={`/service/${recommendedService.slug}`}
                  ...
                  <div className="p-5 border border-slate-200 rounded-xl">
                    ...
                    <p className="text-sm text-slate-600 mb-3">
                      Thorough cleaning for empty homes &mdash; inside cabinets, inside the fridge and oven, interior windows, and every surface. Designed to satisfy property management checklists and maximize your chances of a smooth deposit process.
                    </p>
                    <Link
